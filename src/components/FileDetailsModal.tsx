...
export function FileDetailsModal({
  ...
  useEffect(() => {
    ...
    if (isImage(file.name) || isVideo(file.name)) {
      ...
      fetch(
        `${appConfig.apiBaseUrl}/api/s3/metadata?path=${encodeURIComponent(
          fullPath
        )}`
      )
  ...
  const hasPreview =
    isImage(file.name) || isVideo(file.name) || isPDF(file.name);
  const previewUrl = hasPreview
    ? `${appConfig.apiBaseUrl}/api/s3/preview?path=${encodeURIComponent(
        `${bucketName}/${file.key}`
      )}&mw=400&mh=400`
    : null;
    ...
    <div className={styles.overlay} onClick={onClose}>
      ...
      >
        ...
        <div className={styles.body}>
          <div
            className={`${fileDetailsStyles.content} ${
              !showProperties ? fileDetailsStyles.contentFullHeight : ""
            }`}
          >
            {/* Preview */}
            <div
              className={`${fileDetailsStyles.previewContainer} ${
                !showProperties ? fileDetailsStyles.previewContainerFull : ""
              }`}
            >
            ...
            {showProperties && (
              <div className={fileDetailsStyles.properties}>
                ...
                <div className={fileDetailsStyles.propertyRow}>
                  ...
                  <span className={fileDetailsStyles.propertyValue}>
                    {extension ? extension.toUpperCase() : "Unknown"}
                  </span>
                </div>
                  ...
                  <div className={fileDetailsStyles.propertyRow}>
                    ...
                    <span className={fileDetailsStyles.propertyValue}>
                      {isLoadingMetadata ? (
                        <Loader2
                          size={14}
                          className={fileDetailsStyles.spinner}
                        />
                ...
                <div className={fileDetailsStyles.propertyRow}>
                  ...
                  <span
                    className={fileDetailsStyles.propertyValue}
                    title={`${bucketName}/${file.key}`}
                  >
